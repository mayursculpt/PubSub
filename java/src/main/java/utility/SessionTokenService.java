...
public class SessionTokenService {
    ...
    public APISessionCredentials login(String loginEndpoint, String user, String pwd, boolean useProvidedLoginUrl) throws Exception {
   	CloseableHttpClient httpClient=HttpClients.createDefault();
    	      URL endpoint = new URL(new URL(loginEndpoint), SERVICES_SOAP_PARTNER_ENDPOINT);
    	      HttpPost post = new HttpPost(endpoint.toURI());
    	      post.addHeader("Content-Type", "text/xml");
    	      post.addHeader("SOAPAction", "''");
    	      post.addHeader("PrettyPrint", "Yes");
    	      post.setEntity(new StringEntity(soapXmlForLoginStr(user, pwd), "text/xml", "UTF-8"));
    	      System.out.println("Connect httpClient "+endpoint.toURI());
    	      CloseableHttpResponse response = httpClient.execute(post);
    	      HttpEntity entity = response.getEntity();
    	      LoginResponseParser parser = parse(entity);
//    	URL endpoint;
...
//        LoginResponseParser parser = parse(response);
    ...
    private static LoginResponseParser parse(HttpEntity response) throws Exception {
    	 LoginResponseParser parser  = new LoginResponseParser();;
    	try {
            SAXParserFactory spf = SAXParserFactory.newInstance();
            spf.setFeature("http://xml.org/sax/features/external-general-entities", false);
            ...
            spf.setNamespaceAware(true);
            SAXParser saxParser = spf.newSAXParser();
            saxParser.parse(response.getContent(), parser);

            return parser;
        } catch (SAXException e) {
            throw new Exception(String.format("Unable to login: %s::%s",  parser.faultstring));
        }
    }
    ...
    private static  String soapXmlForLoginStr(String username, String password)
    	      throws UnsupportedEncodingException {
    	    return (ENV_START + "  <urn:login>" + "    <urn:username>" + username + "</urn:username>"
    	        + "    <urn:password>" + password + "</urn:password>" + "  </urn:login>" + ENV_END);
    	  }
}
