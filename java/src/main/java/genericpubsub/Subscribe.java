...
public class Subscribe extends CommonContext {
    ...
    private  ExecutorService eventProcessingExecutors;
    ...
    public Subscribe(ExampleConfigurations exampleConfigurations) {
        ...
        this.keepLiveScheduler = Executors.newScheduledThreadPool(1);
        this.eventProcessingExecutors = Executors.newFixedThreadPool(3);
    }
    public void doSubscribe(ExampleConfigurations exampleConfigurations) {
    	 isActive.set(true);
         this.exampleConfigurations = exampleConfigurations;
         this.BATCH_SIZE = Math.min(5, exampleConfigurations.getNumberOfEventsToSubscribeInEachFetchRequest());
         this.responseStreamObserver = getDefaultResponseStreamObserver();
    ...
    public void startSubscription() {
        ...
        fetch(this.responseStreamObserver,BATCH_SIZE,busTopicName, replayPreset, customReplayId);
        ...
        int i=0;
        while(isActive.get() && i<4) {
        	i++;
            waitInMillis(5_000);
            logger.info("Subscription Active. Received " + receivedEvents.get() + " events.");
        }
       // if(isActive.get())
       // keepLiveScheduler.schedule(new KeepLive(replayPreset, customReplayId), 2000, TimeUnit.MILLISECONDS);
//        KeepLive k = new KeepLive(BATCH_SIZE,busTopicName,replayPreset, customReplayId);
...
//        t.start();
     ...
     */
    public void fetch(StreamObserver<FetchResponse> responseStreamObserver,int providedBatchSize, String providedTopicName, ReplayPreset providedReplayPreset, ByteString providedReplayId) {
       System.out.println("Fetch: "+ providedBatchSize);
    ...
    private class RetryRequestSender implements Runnable {
        ...
        @Override
        public void run() {
            fetch(responseStreamObserver, BATCH_SIZE, busTopicName, retryReplayPreset, retryReplayId);
            logger.info("Retry FetchRequest Sent.");
        }
    }
    private class KeepLive implements Runnable {
    	private final ReplayPreset replayPreset;
    	private final ByteString customReplayId;
    	private StreamObserver<FetchResponse> responseStreamObserverLcl;
    	String topicName;
        public KeepLive(int providedBatchSize, String providedTopicName,ReplayPreset replayPreset, ByteString customReplayId) {
            this.replayPreset = replayPreset;
            this.customReplayId = customReplayId;
            this.topicName=providedTopicName;
            System.out.println("KEep Live Init");
        }
        ...
        public void run() {
        	while(true){
        		...
        		responseStreamObserverLcl = getDefaultResponseStreamObserver();
        		System.out.println("Keeplive Run called new");
                fetch(responseStreamObserverLcl,BATCH_SIZE, topicName, replayPreset, customReplayId);
                logger.info("FetchRequest Sent.");
        	}
        }
    }
